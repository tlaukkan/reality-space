...
export class StorageClient {
    ...
    constructor(dimensionName: string, region: string, storageUrl: string, cdnUrl: string, idToken: string) {
    ...
    async getRootEntitiesFromCdn(): Promise<string> {
        const entitiesXmlUrl = this.cdnUrl + "dimensions/" + this.dimensionName + "/processors/" + this.region + "/entities.xml";
    ...
    private async request(method: string, path: string, successStatuses: Array<number>) {
        const response = (await fetch(this.storageUrl + "dimensions/" + this.dimensionName + "/processors/" + this.region + path, {
            method: method,
            headers: {"Authorization": "Bearer " + this.idToken, "Request-ID": uuid.v4()}
        }));
