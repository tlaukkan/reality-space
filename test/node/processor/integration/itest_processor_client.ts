...
import {
    ...
    PUBLIC_TEST_CLUSTER_PROCESSOR_0_0_200_NAME, PUBLIC_TEST_CLUSTER_PROCESSOR_URL, PUBLIC_TEST_CLUSTER_STORAGE_URL
} from "../../../test";
...
describe('Integration Test Client', () => {
    ...
    it('Should connect client to aframe-dataspace-0-0-100.herokuapp.com.', async () => {
        const client = new RealityClient(DEFAULT_DIMENSION, PUBLIC_TEST_CLUSTER_PROCESSOR_0_0_200_NAME, PUBLIC_TEST_CLUSTER_PROCESSOR_URL, PUBLIC_TEST_CLUSTER_STORAGE_URL, PUBLIC_TEST_CLUSTER_CDN_URL, createTestIdToken());
        client.newWebSocket = (url:string, protocol:string) => { return new w3cwebsocket(url, protocol) as any};
        await client.connect();
        client.close();
    });
