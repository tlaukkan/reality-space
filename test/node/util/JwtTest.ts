...
describe('JWT Test', () => {
    ...
    it('It should create and validate id token.', async () => {
        ...
        const token = createIdToken("issuer", "1", "test@test", "test", privateKeyEncoded);
        const claims = validateIdToken(token, publicKeyEncoded);
        console.log(claims);
        expect(claims.get("iss")).equal("issuer");
