...
export async function startTestServer(server: DataSpaceServer) {
    ...
    server = new DataSpaceServer('127.0.0.1', 8889, processor, storageRestService, [new IdTokenIssuer("test-issuer", "")]);
    server.listen();
...
export function resetStorage(server: DataSpaceServer) {
    server.storageApi.storage.clear();
    server.storageApi.storage.init();
    const principal = new Principal("", "", "", "1", "test");
    server.storageApi.storage.addUser(principal, principal.userId, principal.userName);
}
